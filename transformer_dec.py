...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, max_len, d_model, num_heads, dropout=0., gain=1., is_first=False, num_cross_heads=None):
        ...
        self.slot_self_attn = MultiHeadAttention(d_model, num_heads, dropout, gain)
        ...
        self.slot_self_attn_layer_norm = nn.LayerNorm(d_model)
    ...
    def forward(self, input, encoder_output, causal_mask=True):
        ...
        if self.is_first:
            ...
            input = input + x
            # input = input + self.attn_dropout(x)
        else:
        ...
        encoder_output_slot = self.slot_self_attn_layer_norm(encoder_output)
        slots_output = self.slot_self_attn(encoder_output_slot, encoder_output_slot, encoder_output_slot)
        encoder_output_slot = encoder_output_slot + slots_output
        # encoder_output_slot = encoder_output_slot + self.attn_dropout(slots_output)
        
        x = self.encoder_decoder_attn_layer_norm(input)
        # x = self.encoder_decoder_attn(x, encoder_output, encoder_output)
        x = self.encoder_decoder_attn(x, encoder_output_slot, encoder_output_slot)
        input = input + x
        # input = input + self.attn_dropout(x)
